...
public class CameraPush {
	...
	protected double framerate=FRAME_RATE;// 帧率
	protected int bitrate;// 比特率

	// 音频参数  想要录制音频，这三个参数必须有：audioChannels > 0 && audioBitrate > 0 && sampleRate > 0
	...
	public CameraPush from() throws Exception {
		...
		try {
			...
			logger.error("与拉流IP：   " + cameraPojo.getIp() + "   端口：   554    建立TCP连接！");
		} catch (IOException e) {
			grabber.stop();
			grabber.close();
			rtspSocket.close();
		...
		try {
			...
			logger.error("与推流IP：   " + config.getPush_host() + "   端口：   " + config.getPush_port() + " 建立TCP连接！");
		} catch (IOException e) {
			grabber.stop();
			grabber.close();
			rtspSocket.close();
...
//				logger.info("  no_frame_index   "+no_frame_index+"  err_index   "+err_index);
